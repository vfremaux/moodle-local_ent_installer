...
class core_cohort_external extends external_api {
    ...
    public static function get_cohorts($cohortids = array()) {
        ...
        // PATCH+ : millesim reorder cohorts / ENT.
        /*
        if (empty($cohortids)) {
        ...
        */
        ...
        if (empty($cohortids)) {
            $cohorts = $DB->get_records('cohort', [], "SUBSTR(name, 1, {$config->cohort_sort_prefix_length}) DESC, idnumber ASC");
        } else {
            $cohorts = $DB->get_records_list('cohort', 'id', $params['cohortids'], "SUBSTR(name, 1, {$config->cohort_sort_prefix_length}) DESC, idnumber ASC");
        }
        // /PATCH.
    ...
    public static function search_cohorts($query, $context, $includes = 'parents', $limitfrom = 0, $limitnum = 25) {
        ...
        // PATCH+ : Millesim resort cohorts / ENT;
        if (!is_dir($CFG->dirroot.'/local/ent_installer')) {
            ...
            include_once($CFG->dirroot.'/customscripts/cohort/locallib.php');
            if ($includes == 'self') {
                $results = cohort_get_sorted_cohorts($context->id, $limitfrom, $limitnum, $query);
                $results = $results['cohorts'];
            } else if ($includes == 'parents') {
                $results = cohort_get_sorted_cohorts($context->id, $limitfrom, $limitnum, $query);
                $results = $results['cohorts'];
                if (!$context instanceof context_system) {
                    $results = array_merge($results, cohort_get_available_sorted_cohorts($context, COHORT_ALL, $limitfrom, $limitnum, $query));
                }
            } else if ($includes == 'all') {
                $results = cohort_get_all_sorted_cohorts($limitfrom, $limitnum, $query);
                $results = $results['cohorts'];
            } else {
                throw new coding_exception('Invalid parameter value for \'includes\'.');
            }
