...
require_once($CFG->dirroot.'/customscripts/cohort/locallib.php');
...
// CHANGE
// Sort by prefix
// uasort($cohorts['cohorts'], 'cohort_sort_by_prefix');
...
foreach($cohorts['cohorts'] as $cohort) {
    ...
    if (empty($cohort->component)) {
        ...
        $cohortlisturl = new moodle_url('/local/ent_installer/cohort/list.php', array('id' => $cohort->id, 'contextid' => $context->id));
        $membercount = $DB->count_records('cohort_members', array('cohortid' => $cohort->id));
        $line[] = (!$membercount) ? 0 : $membercount. ' <a href="'.$cohortlisturl.'">'.get_string('seedetail', 'local_ent_installer').'</a>';
    }
    // /CHANGE
