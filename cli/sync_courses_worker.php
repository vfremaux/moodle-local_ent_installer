...
if ($options['help'] || empty($options['nodes'])) {
    $help = "
Command line ENT Sync worker for courses.
...
foreach ($nodes as $nodeid) {
    ...
    if (!empty($options['logroot'])) {
        $logfile = $options['logroot'].'/ent_sync_courses_'.$host->shortname;
    ...
    if (isset($LOG)) {
        fputs($LOG, "Starting Courses worker for nodes {$options['nodes']}\n");
    };

    mtrace("\nStarting process for node $nodeid");

    $host = $DB->get_record('local_vmoodle', array('id' => $nodeid));
    $cmd = "php {$CFG->dirroot}/local/ent_installer/cli/sync_courses.php {$debug} --host={$host->vhostname} ";
    ...
    if ($return) {
        ...
        echo "Courses Worker execution error on {$host->vhostname}:\n";
        echo implode("\n", $output)."\n";
