...
$filterclause = (!empty($filter)) ? " AND (lastname LIKE '%$filter%' OR firstname LIKE '%$filter%')  " : '';

$select = " auth = ? AND deleted = 0 AND mnethostid = ? $filterclause";
$params = array($config->real_used_auth, $CFG->mnet_localhost_id);

if ($users = $DB->get_records_select('user', $select, $params, 'username', 'id, '.get_all_user_name_fields(true, ''))) {
